...
impl From<vk::Result> for GpuQkRopeError {
    fn from(v: vk::Result) -> Self { Self::Vk(v) }
}
impl From<std::str::Utf8Error> for GpuQkRopeError {
    fn from(v: std::str::Utf8Error) -> Self { Self::Utf8(v) }
}
impl From<std::ffi::NulError> for GpuQkRopeError {
    fn from(v: std::ffi::NulError) -> Self { Self::CString(v) }
}
...
impl CachedGpuQkRopeRunner {
    pub fn new_with_context(
        ...
        let q_buffer = create_host_visible_buffer(&instance, &device, physical_device, (query_len * 4) as u64, vk::BufferUsageFlags::STORAGE_BUFFER)?;
        let k_buffer = create_host_visible_buffer(&instance, &device, physical_device, (key_len * 4) as u64, vk::BufferUsageFlags::STORAGE_BUFFER)?;
        let q_weight_buffer = create_host_visible_buffer(&instance, &device, physical_device, (head_dim * 4) as u64, vk::BufferUsageFlags::STORAGE_BUFFER)?;
        let k_weight_buffer = create_host_visible_buffer(&instance, &device, physical_device, (head_dim * 4) as u64, vk::BufferUsageFlags::STORAGE_BUFFER)?;
        let cos_buffer = create_host_visible_buffer(&instance, &device, physical_device, (head_dim * 4) as u64, vk::BufferUsageFlags::STORAGE_BUFFER)?;
        let sin_buffer = create_host_visible_buffer(&instance, &device, physical_device, (head_dim * 4) as u64, vk::BufferUsageFlags::STORAGE_BUFFER)?;
        let q_out_buffer = create_host_visible_buffer(&instance, &device, physical_device, (query_len * 4) as u64, vk::BufferUsageFlags::STORAGE_BUFFER)?;
        let k_out_buffer = create_host_visible_buffer(&instance, &device, physical_device, (key_len * 4) as u64, vk::BufferUsageFlags::STORAGE_BUFFER)?;
        ...
        let descriptor_layout_bindings = [
            binding(0), binding(1), binding(2), binding(3), binding(4), binding(5), binding(6), binding(7),
        ];
        ...
        let pipeline = unsafe {
            device.create_compute_pipelines(vk::PipelineCache::null(), &compute_ci, None).map_err(|(_, err)| GpuQkRopeError::Vk(err))?[0]
        };
        let pool_sizes = [vk::DescriptorPoolSize::default().ty(vk::DescriptorType::STORAGE_BUFFER).descriptor_count(8)];
        ...
        let writes = infos.iter().enumerate().map(|(i, info)| {
        ...
        unsafe {
            ...
            let push_constants = [num_query_heads as u32, num_key_value_heads as u32, head_dim as u32];
            ...
            device.cmd_dispatch(command_buffer, (num_query_heads + num_key_value_heads) as u32, 1, 1);
...
fn compile_shader(path: &Path) -> Result<PathBuf, GpuQkRopeError> {
    ...
    let shader_stem = path.file_stem().and_then(|stem| stem.to_str()).unwrap_or("qk-rope");
    ...
    if !output.status.success() { return Err(GpuQkRopeError::Process(String::from_utf8_lossy(&output.stderr).to_string())); }
...
fn create_host_visible_buffer(instance: &Instance, device: &Device, physical_device: vk::PhysicalDevice, size: u64, usage: vk::BufferUsageFlags) -> Result<BufferAllocation, GpuQkRopeError> {
...
fn find_memory_type(props: &vk::PhysicalDeviceMemoryProperties, filter: u32, flags: vk::MemoryPropertyFlags) -> Result<u32, GpuQkRopeError> {
    ...
    Err(GpuQkRopeError::Shape("missing compatible Vulkan memory type".to_string()))
}

fn write_f32_buffer(buffer: &BufferAllocation, data: &[f32]) -> Result<(), GpuQkRopeError> {
    let byte_len = std::mem::size_of_val(data) as u64;
    if byte_len > buffer.size { return Err(GpuQkRopeError::Shape(format!("f32 write {} bytes exceeds mapped buffer size {}", byte_len, buffer.size))); }
...
fn zero_buffer(buffer: &BufferAllocation, byte_len: usize) -> Result<(), GpuQkRopeError> {
    if byte_len as u64 > buffer.size { return Err(GpuQkRopeError::Shape(format!("zero {} bytes exceeds mapped buffer size {}", byte_len, buffer.size))); }
...
fn read_f32_buffer(buffer: &BufferAllocation, len: usize) -> Result<Vec<f32>, GpuQkRopeError> {
    ...
    if byte_len as u64 > buffer.size { return Err(GpuQkRopeError::Shape(format!("f32 read {} bytes exceeds mapped buffer size {}", byte_len, buffer.size))); }
    let mut output = vec![0f32; len];
    unsafe { std::ptr::copy_nonoverlapping(buffer.mapped_ptr, output.as_mut_ptr() as *mut u8, byte_len); }
    Ok(output)
}
