...
impl std::fmt::Display for GpuSwigluError {
...
impl std::error::Error for GpuSwigluError {}
impl From<std::io::Error> for GpuSwigluError { fn from(v: std::io::Error) -> Self { Self::Io(v) } }
impl From<vk::Result> for GpuSwigluError { fn from(v: vk::Result) -> Self { Self::Vk(v) } }
impl From<ash::LoadingError> for GpuSwigluError { fn from(v: ash::LoadingError) -> Self { Self::Load(v) } }
impl From<std::str::Utf8Error> for GpuSwigluError { fn from(v: std::str::Utf8Error) -> Self { Self::Utf8(v) } }
impl From<std::ffi::NulError> for GpuSwigluError { fn from(v: std::ffi::NulError) -> Self { Self::CString(v) } }

pub fn run_swiglu_with_output(
    gate: &[f32],
    up: &[f32],
    reference: Option<&[f32]>,
) -> Result<(Vec<f32>, GpuSwigluReport), GpuSwigluError> {
    let context = SharedGpuPackedContext::new().map_err(map_context_error)?;
    let (mut runner, compile_duration) = CachedGpuSwigluRunner::new_with_context(context, gate.len())?;
    let (output, mut report) = runner.run_with_output(gate, up, reference)?;
    report.compile_duration = compile_duration;
    Ok((output, report))
}

pub struct CachedGpuSwigluRunner {
    ...
    fence: vk::Fence,
    gate_buffer: BufferAllocation,
    up_buffer: BufferAllocation,
    output_buffer: BufferAllocation,
}

impl CachedGpuSwigluRunner {
    pub fn new_with_context(
        context: Arc<SharedGpuPackedContext>,
        len: usize,
    ) -> Result<(Self, Duration), GpuSwigluError> {
        ...
        let shader_path = compile_shader(Path::new("shaders/swiglu.comp"))?;

        let gate_buffer = create_host_visible_buffer(&instance, &device, physical_device, (len * 4) as u64, vk::BufferUsageFlags::STORAGE_BUFFER)?;
        let up_buffer = create_host_visible_buffer(&instance, &device, physical_device, (len * 4) as u64, vk::BufferUsageFlags::STORAGE_BUFFER)?;
        let output_buffer = create_host_visible_buffer(&instance, &device, physical_device, (len * 4) as u64, vk::BufferUsageFlags::STORAGE_BUFFER)?;
        zero_buffer(&gate_buffer, len * 4)?;
        zero_buffer(&up_buffer, len * 4)?;
        ...
        let push_range = [vk::PushConstantRange::default().stage_flags(vk::ShaderStageFlags::COMPUTE).offset(0).size(4)];
        ...
        let pipeline = unsafe { device.create_compute_pipelines(vk::PipelineCache::null(), &compute_ci, None).map_err(|(_, err)| GpuSwigluError::Vk(err))?[0] };
        let pool_sizes = [vk::DescriptorPoolSize::default().ty(vk::DescriptorType::STORAGE_BUFFER).descriptor_count(3)];
        ...
        let gate_info = [vk::DescriptorBufferInfo::default().buffer(gate_buffer.buffer).offset(0).range(vk::WHOLE_SIZE)];
        let up_info = [vk::DescriptorBufferInfo::default().buffer(up_buffer.buffer).offset(0).range(vk::WHOLE_SIZE)];
        let output_info = [vk::DescriptorBufferInfo::default().buffer(output_buffer.buffer).offset(0).range(vk::WHOLE_SIZE)];
        let writes = [
            vk::WriteDescriptorSet::default().dst_set(descriptor_set).dst_binding(0).descriptor_type(vk::DescriptorType::STORAGE_BUFFER).buffer_info(&gate_info),
            vk::WriteDescriptorSet::default().dst_set(descriptor_set).dst_binding(1).descriptor_type(vk::DescriptorType::STORAGE_BUFFER).buffer_info(&up_info),
            vk::WriteDescriptorSet::default().dst_set(descriptor_set).dst_binding(2).descriptor_type(vk::DescriptorType::STORAGE_BUFFER).buffer_info(&output_info),
        ];
        ...
        unsafe {
            ...
            device.cmd_push_constants(command_buffer, pipeline_layout, vk::ShaderStageFlags::COMPUTE, 0, push_bytes);
    ...
    pub fn run_with_output(
        ...
        if gate.len() != self.len || up.len() != self.len {
            return Err(GpuSwigluError::Shape(format!("gate len {} and up len {} must both match runner len {}", gate.len(), up.len(), self.len)));
        }
        let upload_started = Instant::now();
        write_f32_buffer(&self.gate_buffer, gate)?;
        write_f32_buffer(&self.up_buffer, up)?;
...
impl Drop for CachedGpuSwigluRunner {
    fn drop(&mut self) {
        unsafe {
            ...
            destroy_buffer(&self.device, self.gate_buffer);
            destroy_buffer(&self.device, self.up_buffer);
...
fn compile_shader(path: &Path) -> Result<PathBuf, GpuSwigluError> {
    let temp_dir = std::env::var_os("TMPDIR").map(PathBuf::from).unwrap_or_else(|| PathBuf::from(".tmp"));
    std::fs::create_dir_all(&temp_dir)?;
    let shader_stem = path.file_stem().and_then(|stem| stem.to_str()).unwrap_or("swiglu");
    ...
    if !output.status.success() { return Err(GpuSwigluError::Process(String::from_utf8_lossy(&output.stderr).to_string())); }
    std::fs::rename(&tmp, &out)?;
    Ok(out)
}

fn create_shader_module(device: &Device, path: &Path) -> Result<vk::ShaderModule, GpuSwigluError> {
...
fn create_host_visible_buffer(instance: &Instance, device: &Device, physical_device: vk::PhysicalDevice, size: u64, usage: vk::BufferUsageFlags) -> Result<BufferAllocation, GpuSwigluError> {
...
fn find_memory_type(props: &vk::PhysicalDeviceMemoryProperties, filter: u32, flags: vk::MemoryPropertyFlags) -> Result<u32, GpuSwigluError> {
    ...
    Err(GpuSwigluError::Shape("no matching Vulkan memory type found".to_string()))
}

fn write_f32_buffer(buffer: &BufferAllocation, data: &[f32]) -> Result<(), GpuSwigluError> {
    let byte_len = std::mem::size_of_val(data) as u64;
    if byte_len > buffer.size { return Err(GpuSwigluError::Shape(format!("f32 write {} bytes exceeds mapped buffer size {}", byte_len, buffer.size))); }
    unsafe { std::ptr::copy_nonoverlapping(data.as_ptr() as *const u8, buffer.mapped_ptr, byte_len as usize); }
    Ok(())
}

fn zero_buffer(buffer: &BufferAllocation, byte_len: usize) -> Result<(), GpuSwigluError> {
    if byte_len as u64 > buffer.size { return Err(GpuSwigluError::Shape(format!("zero {} bytes exceeds mapped buffer size {}", byte_len, buffer.size))); }
    unsafe { std::ptr::write_bytes(buffer.mapped_ptr, 0, byte_len); }
    Ok(())
}

fn read_f32_buffer(buffer: &BufferAllocation, len: usize) -> Result<Vec<f32>, GpuSwigluError> {
    let mut out = vec![0.0f32; len];
    unsafe { std::ptr::copy_nonoverlapping(buffer.mapped_ptr as *const f32, out.as_mut_ptr(), len); }
    Ok(out)
}
...
fn map_context_error(error: crate::gpu::packed_matvec::GpuPackedMatvecError) -> GpuSwigluError {
    match error {
        crate::gpu::packed_matvec::GpuPackedMatvecError::Io(err) => GpuSwigluError::Io(err),
        crate::gpu::packed_matvec::GpuPackedMatvecError::Vk(err) => GpuSwigluError::Vk(err),
        crate::gpu::packed_matvec::GpuPackedMatvecError::Load(err) => GpuSwigluError::Load(err),
        crate::gpu::packed_matvec::GpuPackedMatvecError::Utf8(err) => GpuSwigluError::Utf8(err),
        crate::gpu::packed_matvec::GpuPackedMatvecError::Process(err) => GpuSwigluError::Process(err),
        crate::gpu::packed_matvec::GpuPackedMatvecError::CString(err) => GpuSwigluError::CString(err),
        crate::gpu::packed_matvec::GpuPackedMatvecError::MissingDevice => GpuSwigluError::Shape("no suitable Vulkan device found".to_string()),
        crate::gpu::packed_matvec::GpuPackedMatvecError::MissingQueue => GpuSwigluError::Shape("no suitable compute queue family found".to_string()),
        crate::gpu::packed_matvec::GpuPackedMatvecError::Shape(msg) => GpuSwigluError::Shape(msg),
    }
}
