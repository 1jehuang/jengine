...
impl From<std::io::Error> for GpuSwigluError {
    fn from(v: std::io::Error) -> Self {
        Self::Io(v)
    }
}
impl From<vk::Result> for GpuSwigluError {
    fn from(v: vk::Result) -> Self {
        Self::Vk(v)
    }
}
impl From<ash::LoadingError> for GpuSwigluError {
    fn from(v: ash::LoadingError) -> Self {
        Self::Load(v)
    }
}
impl From<std::str::Utf8Error> for GpuSwigluError {
    fn from(v: std::str::Utf8Error) -> Self {
        Self::Utf8(v)
    }
}
impl From<std::ffi::NulError> for GpuSwigluError {
...
pub fn run_swiglu_with_output(
    ...
    let (mut runner, compile_duration) =
        CachedGpuSwigluRunner::new_with_context(context, gate.len())?;
...
impl CachedGpuSwigluRunner {
    pub fn new_with_context(
        ...
        let gate_buffer = create_host_visible_buffer(
            &instance,
            &device,
            physical_device,
            (len * 4) as u64,
        ...
        let up_buffer = create_host_visible_buffer(
            &instance,
            &device,
            physical_device,
            (len * 4) as u64,
        ...
        let pipeline = unsafe {
            device
                ...
                .map_err(|(_, err)| GpuSwigluError::Vk(err))?[0]
        };
        ...
        let gate_info = [vk::DescriptorBufferInfo::default()
            .buffer(gate_buffer.buffer)
            .offset(0)
            .range(vk::WHOLE_SIZE)];
        let up_info = [vk::DescriptorBufferInfo::default()
            .buffer(up_buffer.buffer)
            .offset(0)
            .range(vk::WHOLE_SIZE)];
    ...
    pub fn run_with_output(
        ...
        if gate.len() != self.len || up.len() != self.len {
            return Err(GpuSwigluError::Shape(format!(
                "gate len {} and up len {} must both match runner len {}",
                gate.len(),
                up.len(),
                self.len
            )));
...
fn compile_shader(path: &Path) -> Result<PathBuf, GpuSwigluError> {
    ...
    if !output.status.success() {
        return Err(GpuSwigluError::Process(
...
) -> Result<BufferAllocation, GpuSwigluError> {
...
fn map_context_error(error: crate::gpu::packed_matvec::GpuPackedMatvecError) -> GpuSwigluError {
    match error {
        ...
        crate::gpu::packed_matvec::GpuPackedMatvecError::Process(err) => {
            GpuSwigluError::Process(err)
        }
        crate::gpu::packed_matvec::GpuPackedMatvecError::CString(err) => {
            GpuSwigluError::CString(err)
        }
        crate::gpu::packed_matvec::GpuPackedMatvecError::MissingDevice => {
            GpuSwigluError::Shape("no suitable Vulkan device found".to_string())
        }
        crate::gpu::packed_matvec::GpuPackedMatvecError::MissingQueue => {
            GpuSwigluError::Shape("no suitable compute queue family found".to_string())
        }
