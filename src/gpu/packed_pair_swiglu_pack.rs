...
impl From<std::io::Error> for GpuPackedPairSwigluPackError {
    fn from(v: std::io::Error) -> Self {
        Self::Io(v)
    }
}
impl From<vk::Result> for GpuPackedPairSwigluPackError {
    fn from(v: vk::Result) -> Self {
        Self::Vk(v)
    }
}
impl From<ash::LoadingError> for GpuPackedPairSwigluPackError {
    fn from(v: ash::LoadingError) -> Self {
        Self::Load(v)
    }
}
impl From<std::str::Utf8Error> for GpuPackedPairSwigluPackError {
    fn from(v: std::str::Utf8Error) -> Self {
        Self::Utf8(v)
    }
}
impl From<std::ffi::NulError> for GpuPackedPairSwigluPackError {
...
pub fn run_packed_pair_swiglu_pack_with_output(
    ...
    let (mut runner, compile_duration) = CachedGpuPackedPairSwigluPackRunner::new_with_context(
        context, code_words, scales, group_size, rows, cols,
    )?;
...
impl CachedGpuPackedPairSwigluPackRunner {
    pub fn new_with_context(
        ...
        let shader_path =
            compile_shader(Path::new("shaders/packed_ternary_pair_swiglu_pack.comp"))?;
        ...
        let pipeline = unsafe {
            device
                ...
                .map_err(|(_, err)| GpuPackedPairSwigluPackError::Vk(err))?[0]
        };
    ...
    pub fn run_with_output(
        ...
        if input.len() != self.cols {
            return Err(GpuPackedPairSwigluPackError::Shape(format!(
                "input len {} does not match cols {}",
                input.len(),
                self.cols
            )));
        }
    ...
    fn record_command_buffer(&self) -> Result<(), GpuPackedPairSwigluPackError> {
        ...
        unsafe {
            ...
            self.device.cmd_dispatch(
                self.command_buffer,
                self.rows.div_ceil(2).div_ceil(64) as u32,
                1,
                1,
            );
...
fn compile_shader(path: &Path) -> Result<PathBuf, GpuPackedPairSwigluPackError> {
    ...
    let shader_stem = path
        ...
        .unwrap_or("packed-pair-swiglu-pack");
    ...
    if !output.status.success() {
        return Err(GpuPackedPairSwigluPackError::Process(
...
fn create_shader_module(
    device: &Device,
    path: &Path,
) -> Result<vk::ShaderModule, GpuPackedPairSwigluPackError> {
...
) -> Result<BufferAllocation, GpuPackedPairSwigluPackError> {
...
fn find_memory_type(
...
) -> Result<u32, GpuPackedPairSwigluPackError> {
    ...
    Err(GpuPackedPairSwigluPackError::Shape(
...
fn write_u32_buffer(
    ...
    data: &[u32],
) -> Result<(), GpuPackedPairSwigluPackError> {
...
fn read_u32_buffer(
    buffer: &BufferAllocation,
    len: usize,
) -> Result<Vec<u32>, GpuPackedPairSwigluPackError> {
