...
impl From<std::io::Error> for GpuSwigluCombinedError { fn from(v: std::io::Error) -> Self { Self::Io(v) } }
impl From<vk::Result> for GpuSwigluCombinedError { fn from(v: vk::Result) -> Self { Self::Vk(v) } }
impl From<ash::LoadingError> for GpuSwigluCombinedError { fn from(v: ash::LoadingError) -> Self { Self::Load(v) } }
impl From<std::str::Utf8Error> for GpuSwigluCombinedError { fn from(v: std::str::Utf8Error) -> Self { Self::Utf8(v) } }
impl From<std::ffi::NulError> for GpuSwigluCombinedError { fn from(v: std::ffi::NulError) -> Self { Self::CString(v) } }
...
pub fn run_swiglu_combined_with_output(
    ...
    let (mut runner, compile_duration) = CachedGpuSwigluCombinedRunner::new_with_context(context, len)?;
...
impl CachedGpuSwigluCombinedRunner {
    pub fn new_with_context(
        ...
        let pipeline = unsafe { device.create_compute_pipelines(vk::PipelineCache::null(), &compute_ci, None).map_err(|(_, err)| GpuSwigluCombinedError::Vk(err))?[0] };
    ...
    pub fn run_with_output(
        ...
        if combined.len() != self.len * 2 {
            return Err(GpuSwigluCombinedError::Shape(format!("combined len {} must match {}", combined.len(), self.len * 2)));
        }
        ...
        Ok((output, GpuSwigluCombinedReport { len: self.len, compile_duration: Duration::ZERO, upload_duration, gpu_duration, download_duration, max_abs_diff, mean_abs_diff }))
    }
    ...
    pub fn run_resident_from_f32_buffer(
        ...
        let upload_duration = self.copy_input_from_buffer(source_context, source_buffer, source_len, source_buffer_size)?;
        let gpu_duration = self.submit_and_wait()?;
        Ok(GpuSwigluCombinedReport { len: self.len, compile_duration: Duration::ZERO, upload_duration, gpu_duration, download_duration: Duration::ZERO, max_abs_diff: 0.0, mean_abs_diff: 0.0 })
    }
    ...
    fn copy_input_from_buffer(&self, source_context: &Arc<SharedGpuPackedContext>, source_buffer: vk::Buffer, source_len: usize, source_buffer_size: u64) -> Result<Duration, GpuSwigluCombinedError> {
        if source_len != self.len * 2 {
            return Err(GpuSwigluCombinedError::Shape(format!("source len {} does not match destination len {}", source_len, self.len * 2)));
        }
        if !Arc::ptr_eq(&self._shared_context, source_context) {
            return Err(GpuSwigluCombinedError::Shape("resident chaining requires runners to share the same Vulkan context".to_string()));
        }
        let byte_len = self.len * 2 * std::mem::size_of::<f32>();
        if byte_len as u64 > source_buffer_size || byte_len as u64 > self.input_buffer.size {
            return Err(GpuSwigluCombinedError::Shape(format!("copy {} bytes exceeds source {} or destination {} buffer size", byte_len, source_buffer_size, self.input_buffer.size)));
        }
...
fn compile_shader(path: &Path) -> Result<PathBuf, GpuSwigluCombinedError> {
    ...
    let shader_stem = path.file_stem().and_then(|stem| stem.to_str()).unwrap_or("swiglu-combined");
    ...
    if !output.status.success() { return Err(GpuSwigluCombinedError::Process(String::from_utf8_lossy(&output.stderr).to_string())); }
    std::fs::rename(&tmp, &out)?;
    Ok(out)
}

fn create_shader_module(device: &Device, path: &Path) -> Result<vk::ShaderModule, GpuSwigluCombinedError> {
...
fn create_host_visible_buffer(instance: &Instance, device: &Device, physical_device: vk::PhysicalDevice, size: u64, usage: vk::BufferUsageFlags) -> Result<BufferAllocation, GpuSwigluCombinedError> {
...
fn find_memory_type(props: &vk::PhysicalDeviceMemoryProperties, filter: u32, flags: vk::MemoryPropertyFlags) -> Result<u32, GpuSwigluCombinedError> {
    ...
    Err(GpuSwigluCombinedError::Shape("no matching Vulkan memory type found".to_string()))
}
...
fn write_f32_buffer(buffer: &BufferAllocation, data: &[f32]) -> Result<(), GpuSwigluCombinedError> {
    ...
    if byte_len > buffer.size { return Err(GpuSwigluCombinedError::Shape(format!("f32 write {} bytes exceeds mapped buffer size {}", byte_len, buffer.size))); }
...
fn zero_buffer(buffer: &BufferAllocation, byte_len: usize) -> Result<(), GpuSwigluCombinedError> {
    if byte_len as u64 > buffer.size { return Err(GpuSwigluCombinedError::Shape(format!("zero {} bytes exceeds mapped buffer size {}", byte_len, buffer.size))); }
    unsafe { std::ptr::write_bytes(buffer.mapped_ptr, 0, byte_len); }
    Ok(())
}

fn read_f32_buffer(buffer: &BufferAllocation, len: usize) -> Result<Vec<f32>, GpuSwigluCombinedError> {
...
fn map_context_error(error: crate::gpu::packed_matvec::GpuPackedMatvecError) -> GpuSwigluCombinedError {
    match error {
        ...
        crate::gpu::packed_matvec::GpuPackedMatvecError::Load(err) => GpuSwigluCombinedError::Load(err),
        crate::gpu::packed_matvec::GpuPackedMatvecError::Utf8(err) => GpuSwigluCombinedError::Utf8(err),
        crate::gpu::packed_matvec::GpuPackedMatvecError::Process(err) => GpuSwigluCombinedError::Process(err),
        crate::gpu::packed_matvec::GpuPackedMatvecError::CString(err) => GpuSwigluCombinedError::CString(err),
        crate::gpu::packed_matvec::GpuPackedMatvecError::MissingDevice => GpuSwigluCombinedError::Shape("no suitable Vulkan device found".to_string()),
        crate::gpu::packed_matvec::GpuPackedMatvecError::MissingQueue => GpuSwigluCombinedError::Shape("no suitable compute queue family found".to_string()),
        crate::gpu::packed_matvec::GpuPackedMatvecError::Shape(msg) => GpuSwigluCombinedError::Shape(msg),
    }
}
