...
impl From<std::io::Error> for GpuSwigluPackF16PairsError {
    fn from(v: std::io::Error) -> Self {
        Self::Io(v)
    }
}
impl From<vk::Result> for GpuSwigluPackF16PairsError {
    fn from(v: vk::Result) -> Self {
        Self::Vk(v)
    }
}
impl From<ash::LoadingError> for GpuSwigluPackF16PairsError {
    fn from(v: ash::LoadingError) -> Self {
        Self::Load(v)
    }
}
impl From<std::str::Utf8Error> for GpuSwigluPackF16PairsError {
    fn from(v: std::str::Utf8Error) -> Self {
        Self::Utf8(v)
    }
}
impl From<std::ffi::NulError> for GpuSwigluPackF16PairsError {
...
pub fn run_swiglu_pack_f16_pairs_with_output(
    ...
    let (mut runner, compile_duration) =
        CachedGpuSwigluPackF16PairsRunner::new_with_context(context, len)?;
...
impl CachedGpuSwigluPackF16PairsRunner {
    pub fn new_with_context(
        ...
        let pipeline = unsafe {
            device
                ...
                .map_err(|(_, err)| GpuSwigluPackF16PairsError::Vk(err))?[0]
        };
    ...
    pub fn run_with_output(
        ...
        let mismatched = reference
            .map(|r| r.iter().zip(output.iter()).filter(|(a, b)| a != b).count())
            .unwrap_or(0);
        Ok((
            output,
            GpuSwigluPackF16PairsReport {
                len: self.len,
                compile_duration: Duration::ZERO,
                upload_duration,
                gpu_duration,
                download_duration,
                max_mismatched_words: mismatched,
            },
        ))
    }
...
fn compile_shader(path: &Path) -> Result<PathBuf, GpuSwigluPackF16PairsError> {
    ...
    let shader_stem = path
        ...
        .unwrap_or("swiglu-pack-f16-pairs");
    ...
    if !output.status.success() {
        return Err(GpuSwigluPackF16PairsError::Process(
...
fn create_shader_module(
    device: &Device,
    path: &Path,
) -> Result<vk::ShaderModule, GpuSwigluPackF16PairsError> {
...
) -> Result<BufferAllocation, GpuSwigluPackF16PairsError> {
...
fn find_memory_type(
...
) -> Result<u32, GpuSwigluPackF16PairsError> {
    ...
    Err(GpuSwigluPackF16PairsError::Shape(
...
fn read_u32_buffer(
    buffer: &BufferAllocation,
    len: usize,
) -> Result<Vec<u32>, GpuSwigluPackF16PairsError> {
    let mut out = vec![0u32; len];
    unsafe {
        std::ptr::copy_nonoverlapping(buffer.mapped_ptr as *const u32, out.as_mut_ptr(), len);
    }
    Ok(out)
}
...
) -> GpuSwigluPackF16PairsError {
    match error {
        crate::gpu::packed_matvec::GpuPackedMatvecError::Io(err) => {
            GpuSwigluPackF16PairsError::Io(err)
        }
        crate::gpu::packed_matvec::GpuPackedMatvecError::Vk(err) => {
            GpuSwigluPackF16PairsError::Vk(err)
        }
        crate::gpu::packed_matvec::GpuPackedMatvecError::Load(err) => {
            GpuSwigluPackF16PairsError::Load(err)
        }
        crate::gpu::packed_matvec::GpuPackedMatvecError::Utf8(err) => {
            GpuSwigluPackF16PairsError::Utf8(err)
        }
        crate::gpu::packed_matvec::GpuPackedMatvecError::Process(err) => {
            GpuSwigluPackF16PairsError::Process(err)
        }
        crate::gpu::packed_matvec::GpuPackedMatvecError::CString(err) => {
            GpuSwigluPackF16PairsError::CString(err)
        }
        crate::gpu::packed_matvec::GpuPackedMatvecError::MissingDevice => {
            GpuSwigluPackF16PairsError::Shape("no suitable Vulkan device found".to_string())
        }
        crate::gpu::packed_matvec::GpuPackedMatvecError::MissingQueue => {
            GpuSwigluPackF16PairsError::Shape("no suitable compute queue family found".to_string())
        }
        crate::gpu::packed_matvec::GpuPackedMatvecError::Shape(msg) => {
            GpuSwigluPackF16PairsError::Shape(msg)
        }
    }
}
