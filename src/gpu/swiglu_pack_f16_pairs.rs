...
pub struct GpuSwigluPackF16PairsReport {
    pub len: usize,
    pub compile_duration: Duration,
    pub upload_duration: Duration,
    pub gpu_duration: Duration,
    pub download_duration: Duration,
    pub max_mismatched_words: usize,
}

#[derive(Debug)]
pub enum GpuSwigluPackF16PairsError {
    Io(std::io::Error),
    Vk(vk::Result),
...
impl std::fmt::Display for GpuSwigluPackF16PairsError {
...
impl std::error::Error for GpuSwigluPackF16PairsError {}
impl From<std::io::Error> for GpuSwigluPackF16PairsError { fn from(v: std::io::Error) -> Self { Self::Io(v) } }
impl From<vk::Result> for GpuSwigluPackF16PairsError { fn from(v: vk::Result) -> Self { Self::Vk(v) } }
impl From<ash::LoadingError> for GpuSwigluPackF16PairsError { fn from(v: ash::LoadingError) -> Self { Self::Load(v) } }
impl From<std::str::Utf8Error> for GpuSwigluPackF16PairsError { fn from(v: std::str::Utf8Error) -> Self { Self::Utf8(v) } }
impl From<std::ffi::NulError> for GpuSwigluPackF16PairsError { fn from(v: std::ffi::NulError) -> Self { Self::CString(v) } }

pub fn run_swiglu_pack_f16_pairs_with_output(
    combined: &[f32],
    reference: Option<&[u32]>,
) -> Result<(Vec<u32>, GpuSwigluPackF16PairsReport), GpuSwigluPackF16PairsError> {
    let len = combined.len() / 2;
    let context = SharedGpuPackedContext::new().map_err(map_context_error)?;
    let (mut runner, compile_duration) = CachedGpuSwigluPackF16PairsRunner::new_with_context(context, len)?;
    let (output, mut report) = runner.run_with_output(combined, reference)?;
    report.compile_duration = compile_duration;
    Ok((output, report))
}

pub struct CachedGpuSwigluPackF16PairsRunner {
...
impl CachedGpuSwigluPackF16PairsRunner {
    pub fn new_with_context(
        context: Arc<SharedGpuPackedContext>,
        len: usize,
    ) -> Result<(Self, Duration), GpuSwigluPackF16PairsError> {
        ...
        let shader_path = compile_shader(Path::new("shaders/swiglu_pack_f16_pairs.comp"))?;

        let input_buffer = create_host_visible_buffer(&instance, &device, physical_device, (len * 2 * 4) as u64, vk::BufferUsageFlags::STORAGE_BUFFER)?;
        let output_buffer = create_host_visible_buffer(&instance, &device, physical_device, (packed_len * 4) as u64, vk::BufferUsageFlags::STORAGE_BUFFER)?;
        ...
        let pipeline = unsafe { device.create_compute_pipelines(vk::PipelineCache::null(), &compute_ci, None).map_err(|(_, err)| GpuSwigluPackF16PairsError::Vk(err))?[0] };
        let pool_sizes = [vk::DescriptorPoolSize::default().ty(vk::DescriptorType::STORAGE_BUFFER).descriptor_count(2)];
        ...
        let input_info = [vk::DescriptorBufferInfo::default().buffer(input_buffer.buffer).offset(0).range(vk::WHOLE_SIZE)];
        let output_info = [vk::DescriptorBufferInfo::default().buffer(output_buffer.buffer).offset(0).range(vk::WHOLE_SIZE)];
        let writes = [
            vk::WriteDescriptorSet::default().dst_set(descriptor_set).dst_binding(0).descriptor_type(vk::DescriptorType::STORAGE_BUFFER).buffer_info(&input_info),
            vk::WriteDescriptorSet::default().dst_set(descriptor_set).dst_binding(1).descriptor_type(vk::DescriptorType::STORAGE_BUFFER).buffer_info(&output_info),
        ];
    ...
    pub fn run_with_output(
        ...
        if combined.len() != self.len * 2 {
            return Err(GpuSwigluPackF16PairsError::Shape(format!("combined len {} must match {}", combined.len(), self.len * 2)));
        }
...
fn compile_shader(path: &Path) -> Result<PathBuf, GpuSwigluPackF16PairsError> {
    let temp_dir = std::env::var_os("TMPDIR").map(PathBuf::from).unwrap_or_else(|| PathBuf::from(".tmp"));
    std::fs::create_dir_all(&temp_dir)?;
    let shader_stem = path.file_stem().and_then(|stem| stem.to_str()).unwrap_or("swiglu-pack-f16-pairs");
    ...
    if !output.status.success() { return Err(GpuSwigluPackF16PairsError::Process(String::from_utf8_lossy(&output.stderr).to_string())); }
    std::fs::rename(&tmp, &out)?;
    Ok(out)
}

fn create_shader_module(device: &Device, path: &Path) -> Result<vk::ShaderModule, GpuSwigluPackF16PairsError> {
...
fn create_host_visible_buffer(instance: &Instance, device: &Device, physical_device: vk::PhysicalDevice, size: u64, usage: vk::BufferUsageFlags) -> Result<BufferAllocation, GpuSwigluPackF16PairsError> {
...
fn find_memory_type(props: &vk::PhysicalDeviceMemoryProperties, filter: u32, flags: vk::MemoryPropertyFlags) -> Result<u32, GpuSwigluPackF16PairsError> {
    ...
    Err(GpuSwigluPackF16PairsError::Shape("no matching Vulkan memory type found".to_string()))
}

fn write_f32_buffer(buffer: &BufferAllocation, data: &[f32]) -> Result<(), GpuSwigluPackF16PairsError> {
    let byte_len = std::mem::size_of_val(data) as u64;
    if byte_len > buffer.size { return Err(GpuSwigluPackF16PairsError::Shape(format!("f32 write {} bytes exceeds mapped buffer size {}", byte_len, buffer.size))); }
    unsafe { std::ptr::copy_nonoverlapping(data.as_ptr() as *const u8, buffer.mapped_ptr, byte_len as usize); }
    Ok(())
}

fn zero_buffer(buffer: &BufferAllocation, byte_len: usize) -> Result<(), GpuSwigluPackF16PairsError> {
    if byte_len as u64 > buffer.size { return Err(GpuSwigluPackF16PairsError::Shape(format!("zero {} bytes exceeds mapped buffer size {}", byte_len, buffer.size))); }
    unsafe { std::ptr::write_bytes(buffer.mapped_ptr, 0, byte_len); }
    Ok(())
}

fn read_u32_buffer(buffer: &BufferAllocation, len: usize) -> Result<Vec<u32>, GpuSwigluPackF16PairsError> {
    let mut out = vec![0u32; len];
    unsafe { std::ptr::copy_nonoverlapping(buffer.mapped_ptr as *const u32, out.as_mut_ptr(), len); }
    Ok(out)
}
...
fn map_context_error(error: crate::gpu::packed_matvec::GpuPackedMatvecError) -> GpuSwigluPackF16PairsError {
    match error {
        crate::gpu::packed_matvec::GpuPackedMatvecError::Io(err) => GpuSwigluPackF16PairsError::Io(err),
        crate::gpu::packed_matvec::GpuPackedMatvecError::Vk(err) => GpuSwigluPackF16PairsError::Vk(err),
        crate::gpu::packed_matvec::GpuPackedMatvecError::Load(err) => GpuSwigluPackF16PairsError::Load(err),
        crate::gpu::packed_matvec::GpuPackedMatvecError::Utf8(err) => GpuSwigluPackF16PairsError::Utf8(err),
        crate::gpu::packed_matvec::GpuPackedMatvecError::Process(err) => GpuSwigluPackF16PairsError::Process(err),
        crate::gpu::packed_matvec::GpuPackedMatvecError::CString(err) => GpuSwigluPackF16PairsError::CString(err),
        crate::gpu::packed_matvec::GpuPackedMatvecError::MissingDevice => GpuSwigluPackF16PairsError::Shape("no suitable Vulkan device found".to_string()),
        crate::gpu::packed_matvec::GpuPackedMatvecError::MissingQueue => GpuSwigluPackF16PairsError::Shape("no suitable compute queue family found".to_string()),
        crate::gpu::packed_matvec::GpuPackedMatvecError::Shape(msg) => GpuSwigluPackF16PairsError::Shape(msg),
    }
}
